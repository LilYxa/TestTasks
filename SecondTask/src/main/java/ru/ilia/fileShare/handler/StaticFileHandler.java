...
import ru.ilia.fileShare.utils.FileUtils;
import ru.ilia.fileShare.utils.HttpUtils;
...
public class StaticFileHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        Path baseDir = Paths.get(AppConfig.STATIC_BASE_DIR).toAbsolutePath().normalize();
        Path requestedFile = baseDir.resolve(relativePath).normalize();

        if (!requestedFile.startsWith(baseDir)) {
            HttpUtils.sendErrorResponse(exchange, 403, "Forbidden");
            return;
        }
